...
export default function RecordDonationPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/proxy/soup-kitchen/donations", {
        method: "POST",
      ...
      router.push("/soup-kitchen/donations");
      router.refresh();
    } catch {
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="mb-6 text-2xl font-bold text-zinc-900">Record Donation</h1>
      <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        <Field label="Donor Name" name="donorName" required />
        <Field label="Item" name="item" required />
